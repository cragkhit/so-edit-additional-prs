	...
	private static final char[] PASSWORD = "unauthorizedpersonelisunauthorized".toCharArray();
	//Cipher salt.
	...
	public static void main(String[] args) throws GeneralSecurityException, FileNotFoundException, IOException {
		 ...
		 * NOTE: Ignore the use of Strings here, it's being used here for the convenience only.
		...
		/*
		 * For demosntration purposes, I make three encryption levels.
		...
		System.out.println("READ: [first player decrypted]\n " + decryptedContent + "\n\n\n");
	...
	private static String encrypt(char[] property) throws GeneralSecurityException {
		...
		String encrypted = Base64.encodeBytes(pbeCipher.doFinal(bytes));
		
		//Cleanup both data-source arrays.
		for (int i = 0; i < property.length; i++) {
			property[i] = 0;
			bytes[i] = 0;
		}
	...
	private static String encrypt(String property) throws GeneralSecurityException {
		return encrypt(property.toCharArray());
	}
